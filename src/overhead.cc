...
extern "C" void misc_sim (int t)
{
  ...
  using gptlmain :: disabled;
  using gptlmain :: initialized;
  using gptlmain :: stackidx;
  using gptlmain :: callstack;
...
namespace overhead {
  ...
  */
  extern "C" int getoverhead (FILE *fp, double *self_ohd, double *parent_ohd)
  {
    ...
    t1 = (*ptr2wtimefunc)();
    for (i = 0; i < 1000; ++i) {
      // Assume there is a need to get the length of the string
      int namelen = strlen (timername);
      hashidx = genhashidx (timername, namelen);
    }
    t2 = (*ptr2wtimefunc)();
